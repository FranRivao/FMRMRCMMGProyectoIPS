...
<head>
    ...
    <style>
        ...
        .contenedor { width: 100%; max-width: 600px; background: white; padding: 30px; border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); }
        
        /* TÍTULO DE LA PÁGINA (Con el color corregido) */
        h2.titulo-pagina {
            text-align: center;
            color: #7E0EC5;
        ...
        input[type="submit"] {
            background-color: #28a745; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; font-size: 16px; width: 100%; margin-top: 10px;
        }
...
<body>
    ...
    <?php
            ...
            if (!$link) die("<div class='mensaje error'>Error conexión</div>");
            
            if (!mysqli_select_db($link, "redBancaria")) die("<div class='mensaje error'>Error BD: redBancaria</div>");
            ...
            if (mysqli_query($link, $query)) {
                ...
                echo "<div class='mensaje error'>Error: " . mysqli_error($link) . "</div>";
            }
            mysqli_close($link);
