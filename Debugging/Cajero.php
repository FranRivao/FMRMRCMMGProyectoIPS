...
<head>
    ...
    <link rel="stylesheet" href="estiloCajero.css">
</head>
<body>
    ...
    <?php
        ...
        if ($action == 'save' && $_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            // Recoger Atributos
                        $saldoDisponible = mysqli_real_escape_string($link, $_POST['varsaldoDisponible']);
            
            if ($is_edit) {
                $query = "UPDATE `cajero` SET ";
                                $query .= "saldoDisponible = '$saldoDisponible'";
                                $query .= " WHERE id = '$id'";
        ...
        if ($action == 'delete' && isset($_GET['id'])) {
            ...
            $query = "DELETE FROM `cajero` WHERE id = '$id'";
            if (mysqli_query($link, $query)) {
    ...
    ?>
        ...
        <table>
            ...
            <tbody>
                <?php
                $res = mysqli_query($link, "SELECT * FROM `cajero`");
                while ($row = mysqli_fetch_assoc($res)) {
                    echo "<tr>";
                    echo "<td>" . $row['id'] . "</td>";
                                        echo "<td>" . $row['saldoDisponible'] . "</td>";
    ...
    <?php 
        ...
        if (isset($_GET['id'])) {
            $is_edit = true;
            $id = mysqli_real_escape_string($link, $_GET['id']);
            $res = mysqli_query($link, "SELECT * FROM `cajero` WHERE id='$id'");
            if ($row = mysqli_fetch_assoc($res)) {
                $val_id = $row['id'];
                                $val_saldoDisponible = $row['saldoDisponible'];
                            }
        }
    ?>
        <h2 class="titulo-pagina">
        ...
        <form action="?action=save" method="post">
            ...
            <div class="form-group">
                <label for="varsaldoDisponible" class="lbl-saldoDisponible">saldoDisponible</label>
                <input type="number" name="varsaldoDisponible" id="varsaldoDisponible" 
                       value="<?php echo $val_saldoDisponible; ?>"
                       step='any' required />
            </div>
